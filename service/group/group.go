...
func (s *GroupService) CreateGroup(ctx context.Context, req *svc.CreateGroupRequest) (*iamentity.Group, error) {
	...
	if req.ParentID != nil {
		...
		if err != nil {
			return nil, errors.WrapError(err, errors.ErrCodeNotFound, "父组织不存在")
		}
		parentGroup = parent

		// 检查层级限制
		if parentGroup.Level >= svc.MaxGroupLevel {
			return nil, errors.NewError(errors.ErrCodeValidation, "组织层级不能超过10级")
		}
	}
	...
	if err := s.groupRepo.Create(ctx, group); err != nil {
		return nil, errors.WrapError(err, errors.ErrCodeDatabase, "保存组织失败")
	}
...
func (s *GroupService) DeleteGroup(ctx context.Context, groupID int64) error {
	...
	if len(children) > 0 {
		return errors.NewError(errors.ErrCodeValidation, "不能删除有子组织的组织，请先处理子组织")
	}
	...
	if len(users) > 0 {
		return errors.NewError(errors.ErrCodeValidation, "不能删除有用户的组织，请先移除用户")
	}
