...
func (v *BusinessValidator) ValidateUserDeletion(ctx context.Context, userID int64) error {
	...
	if user.HasRole(SystemAdminRoleName) {
		...
		if len(adminUsers) <= 1 {
			return errorx.NewError(errorx.Validation, "不能删除最后一个系统管理员")
		}
	}
...
func (v *BusinessValidator) ValidateGroupDeletion(ctx context.Context, groupID int64) error {
	...
	if len(children) > 0 {
		return errorx.NewError(errorx.Validation, "不能删除有子组织的组织")
	}
	...
	if len(users) > 0 {
		return errorx.NewError(errorx.Validation, "不能删除有用户的组织")
	}
...
func (v *BusinessValidator) ValidateRoleUpdate(ctx context.Context, roleID int64, req *UpdateRoleRequest) error {
	...
	// 2. 系统角色不能修改
	if role.IsSystem {
		return errorx.NewError(errorx.Validation, "系统角色不能被修改")
	}
...
func (v *BusinessValidator) validateUsernameUniqueness(ctx context.Context, username string) error {
	...
	if err != nil && !errorx.IsNotFound(err) {
		return errorx.WrapError(err, errorx.Database, "检查用户名失败")
	}
	if existingUser != nil {
		return errorx.NewError(errorx.Validation, "用户名已存在")
	}
	return nil
}
...
func (v *BusinessValidator) validateEmailUniqueness(ctx context.Context, email string) error {
	...
	if err != nil && !errorx.IsNotFound(err) {
		return errorx.WrapError(err, errorx.Database, "检查邮箱失败")
	}
	if existingUser != nil {
		return errorx.NewError(errorx.Validation, "邮箱已存在")
	}
	return nil
}
...
func (v *BusinessValidator) validatePasswordStrength(password string) error {
	...
	if len(password) > MaxPasswordLength {
		return errorx.NewError(errorx.Validation, "密码长度不能超过255个字符")
	}
...
func (v *BusinessValidator) validateAvatarURL(avatar string) error {
	if len(avatar) > 500 {
		return errorx.NewError(errorx.Validation, "头像URL长度不能超过500个字符")
	}
	// 可以添加URL格式验证
	return nil
}
...
func (v *BusinessValidator) validateParentGroup(ctx context.Context, parentID int64) error {
	...
	if err != nil {
		return errorx.WrapError(err, errorx.NotFound, "父组织不存在")
	}
	if parent.Level >= MaxGroupLevel {
		return errorx.NewError(errorx.Validation, "组织层级不能超过10级")
	}
	return nil
}
...
func (v *BusinessValidator) validateGroupParentChange(ctx context.Context, group *iamentity.Group, newParentID *int64) error {
	if newParentID != nil {
		...
		if *newParentID == group.GetID() {
			return errorx.NewError(errorx.Validation, "不能将组织设置为自己的父组织")
		}

		// 检查新父组织是否存在
		newParent, err := v.groupRepo.GetByID(ctx, *newParentID)
		if err != nil {
			return errorx.WrapError(err, errorx.NotFound, "新父组织不存在")
		}

		// 不能设置为自己的子组织
		if newParent.IsDescendantOf(group) {
			return errorx.NewError(errorx.Validation, "不能将组织移动到其子组织下")
		}

		// 检查新父组织层级
		if newParent.Level >= MaxGroupLevel {
			return errorx.NewError(errorx.Validation, "目标组织层级过深")
		}
	}
	return nil
}
...
func (v *BusinessValidator) validateRoleNameUniqueness(ctx context.Context, name string) error {
	...
	if err != nil && !errorx.IsNotFound(err) {
		return errorx.WrapError(err, errorx.Database, "检查角色名称失败")
	}
	if existingRole != nil {
		return errorx.NewError(errorx.Validation, "角色名称已存在")
	}
	return nil
}
