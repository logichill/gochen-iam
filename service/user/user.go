...
func (s *UserService) Register(ctx context.Context, req *svc.RegisterRequest) (*iamentity.User, error) {
	...
	if err != nil && !errorx.IsNotFound(err) {
		return nil, errorx.WrapError(err, errorx.Database, "检查用户名失败")
	}
	if existingUser != nil {
		return nil, errorx.NewError(errorx.Validation, "用户名已存在")
	}

	// 3. 检查邮箱是否已存在
	existingUser, err = s.userRepo.FindByEmail(ctx, req.Email)
	if err != nil && !errorx.IsNotFound(err) {
		return nil, errorx.WrapError(err, errorx.Database, "检查邮箱失败")
	}
	if existingUser != nil {
		return nil, errorx.NewError(errorx.Validation, "邮箱已存在")
	}
	...
	if err := s.userRepo.Create(ctx, user); err != nil {
		return nil, errorx.WrapError(err, errorx.Database, "保存用户失败")
	}
...
func (s *UserService) Login(ctx context.Context, req *svc.LoginRequest) (*svc.LoginResponse, error) {
	...
	if req.Username == "" || req.Password == "" {
		return nil, errorx.NewError(errorx.Validation, "用户名和密码不能为空")
	}
	...
	if err != nil {
		if errorx.IsNotFound(err) {
			return nil, errorx.NewError(errorx.NotFound, "用户名或密码错误")
		}
		return nil, errorx.WrapError(err, errorx.Database, "查询用户失败")
	}

	// 3. 验证密码
	if !s.verifyPassword(req.Password, user.Password) {
		return nil, errorx.NewError(errorx.Validation, "用户名或密码错误")
	}

	// 4. 检查用户状态
	if !user.IsActive() {
		return nil, errorx.NewError(errorx.Validation, "用户账户已被禁用")
	}
...
func (s *UserService) ChangePassword(ctx context.Context, userID int64, req *svc.ChangePasswordRequest) error {
	...
	if !s.verifyPassword(req.OldPassword, user.Password) {
		return errorx.NewError(errorx.Validation, "原密码错误")
	}

	// 3. 验证新密码
	if len(req.NewPassword) < svc.MinPasswordLength {
		return errorx.NewError(errorx.Validation, "新密码长度不能少于6个字符")
	}
...
func (s *UserService) UpdateProfile(ctx context.Context, userID int64, req *svc.UpdateUserRequest) (*iamentity.User, error) {
	...
	if req.Email != "" && req.Email != user.Email {
		...
		if existingUser != nil && existingUser.GetID() != userID {
			return nil, errorx.NewError(errorx.Validation, "邮箱已被使用")
		}
